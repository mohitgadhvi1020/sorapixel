...
function CatalogueContent() {
  ...
  const handleGenerate = async () => {
    ...
    trackEvent("generate_started", { type: "catalogue", model: selectedModel, poses: selectedPoses.join(","), background: selectedBg });
    try {
      ...
      if (data.success) {
        const imgs = data.images.filter(i => i.base64);
        setResults(imgs);
        setActiveResult(0);
        trackEvent("image_generated", { type: "catalogue", model: selectedModel, background: selectedBg, poses: selectedPoses.join(","), image_count: imgs.length, duration_ms: Date.now() - startedAt });
      } else {
        trackEvent("generate_failed", { type: "catalogue", reason: data.error || "unknown" });
        setError(data.error || "Generation failed. No tokens were deducted.");
      }
    } catch (e: unknown) {
      stopProgress(false);
      trackEvent("exception", { description: e instanceof Error ? e.message : "catalogue generate failed", where: "catalogue.generate" });
      setError(e instanceof Error ? e.message : "Something went wrong. No tokens were deducted.");
  ...
  const totalTokens = selectedPoses.length * JEWELRY_PRICING.standard.ugcPerPose;
              ...
              <Card padding="none">
                ...
                <div className="p-4 flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="sm" onClick={() => shareToWhatsApp(active.base64, `soraipixel-catalogue-${Date.now()}.png`)}>
                    ...
                    <Button variant="secondary" size="sm" onClick={() => { trackEvent("image_downloaded", { type: "catalogue", label: active.label }); downloadImage(active.base64, `soraipixel-catalogue-${Date.now()}.png`); }}>
              ...
              <>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-[rgba(255,255,255,0.5)]">Catalogue ({totalTokens} tokens)</span>
                  </div>
