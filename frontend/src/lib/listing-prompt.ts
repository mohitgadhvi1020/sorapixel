...
export function buildBrandListingPrompt(config: BrandConfig, jewelryType: string): string {
  const name = config.brandName || "the brand";
  const sections: string[] = [];

  let intro = `You are a product copywriter for ${name}`;
  if (config.website) intro += ` (${config.website})`;
  if (config.tagline) intro += ` — ${config.tagline}`;
  intro += ".";
  sections.push(intro);

  if (config.productType) {
    sections.push(`\nWHAT ${name.toUpperCase()} SELLS: ${config.productType}`);
  }
  ...
  if (voice) {
    let block = `\nBRAND VOICE:\nTone: ${voice.tone || "Professional and clear"}`;
    if (voice.doRules?.length) block += "\n\nDO:\n" + voice.doRules.map((r) => `- ${r}`).join("\n");
    if (voice.dontRules?.length) block += "\n\nDON'T:\n" + voice.dontRules.map((r) => `- ${r}`).join("\n");
    sections.push(block);
  }

  const mat = config.materialRules;
  if (mat && (mat.note || mat.neverSay || mat.bannedWords?.length)) {
    let block = "\nMATERIAL LANGUAGE RULES:";
    if (mat.note) block += `\n${mat.note}`;
    if (mat.neverSay) {
      block += "\n\nNEVER write → ALWAYS write:";
      for (const [bad, good] of Object.entries(mat.neverSay)) {
        block += `\n- "${bad}" → "${good}"`;
      }
    }
    if (mat.bannedWords?.length) block += `\n\nBANNED WORDS (never use anywhere): ${mat.bannedWords.join(", ")}`;
    if (mat.replacements) {
      for (const [bad, good] of Object.entries(mat.replacements)) {
        block += `\nUse "${good}" instead of "${bad}"`;
      }
    }
    if (mat.acceptableExamples?.length) {
      block += "\n\nAcceptable examples:\n" + mat.acceptableExamples.map((ex) => `- ${ex}`).join("\n");
    }
    sections.push(block);
  }
  ...
  if (tax) {
    ...
    if (tax.categories) {
      block += "\nCategories:";
      for (const [cat, subs] of Object.entries(tax.categories)) {
        block += `\n- ${cat}: ${subs.join(", ")}`;
      }
    }
    if (tax.collections?.length) block += `\nCollections: ${tax.collections.join(", ")}`;
    if (tax.occasions?.length) block += `\nOccasions: ${tax.occasions.join(", ")}`;
    sections.push(block);
  }

  const fmt = config.outputFormat;
  const titleRange = fmt?.titleCharRange || [50, 65];
  const titleFmt = (fmt?.titleFormat || "[Descriptive Name] | {brandName}").replace("{brandName}", name);
  const descRange = fmt?.descriptionWordRange || [100, 160];
  const metaRange = fmt?.metaDescCharRange || [140, 155];
  const altMax = fmt?.altTextMaxChars || 125;

  let outBlock = "\nOUTPUT SPECIFICATION:";
  outBlock += `\n\n1. TITLE (${titleRange[0]}-${titleRange[1]} characters):\n   Format: ${titleFmt}`;
  if (fmt?.titleExclude?.length) outBlock += `\n   NEVER include in title: ${fmt.titleExclude.join(", ")}`;
  outBlock += `\n\n2. DESCRIPTION (HTML-ready, ${descRange[0]}-${descRange[1]} words):`;
  if (fmt?.descriptionStructure) outBlock += `\n   ${fmt.descriptionStructure}`;
  outBlock += `\n\n3. META DESCRIPTION (${metaRange[0]}-${metaRange[1]} characters):\n   Action-oriented, includes product type + key differentiator + brand name.`;
  outBlock += `\n\n4. ALT TEXT (under ${altMax} characters):\n   Descriptive, not salesy. No brand name.`;
  outBlock += "\n\n5. ATTRIBUTES (structured):";
  if (fmt?.attributeFields) {
    for (const [field, desc] of Object.entries(fmt.attributeFields)) {
      outBlock += `\n   - ${field}: ${desc}`;
    }
  }
  sections.push(outBlock);

  let prompt = sections.join("\n");
  prompt += `\n\nTASK: Analyze this ${jewelryType} product image and generate a COMPLETE listing following ALL guidelines above.`;
