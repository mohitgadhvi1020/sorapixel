...
export function buildJewelryPackPrompts(backgroundPrompt: string): {
  ...
  return {
    ...
    angle: `Edit this jewelry photograph. ${removeExtras} Replace the background. Recompose as a 45-DEGREE OVERHEAD PRODUCT-ON-SURFACE shot — the jewelry piece is resting naturally on ${backgroundPrompt.toLowerCase().includes("velvet") ? "the velvet" : backgroundPrompt.toLowerCase().includes("marble") ? "the marble" : "the surface"}, shot from a 45-degree elevated angle looking down. Show realistic CONTACT SHADOW where the jewelry meets the surface, giving depth and weight. Add dramatic editorial SIDE-LIGHTING from the left with a subtle SPOT COLOR — a small, soft warm golden light accent reflecting off the metal to create a beautiful highlight bloom on one side of the jewelry. The background surface extends to fill the entire frame edge-to-edge — no dark corners, no vignette. ${backgroundPrompt} CRITICAL PIXEL-PERFECT RULE: Every single stone, facet, prong, bezel, engraving, metal texture, chain link, clasp, and design element must remain EXACTLY identical to the original photograph — same count of stones, same arrangement, same metal color and finish, same proportions. Do NOT add, remove, or alter ANY detail of the jewelry. Only change the viewing angle, surface placement, background, and lighting.`,
  };
}
