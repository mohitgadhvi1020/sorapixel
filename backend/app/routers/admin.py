...
# ---- Client Generated Images Viewer ----

@router.get("/client-images/{client_id}")
async def list_client_images(
    ...
    """List images generated by a given client. Returns public URLs + metadata."""
    sb = get_supabase()
    try:
        result = (
            sb.table("images")
...
async def list_generation_images(
    generation_id: str,
    admin: dict = Depends(require_admin),
):
    """List images belonging to a single generation — used from Recent Activity."""
    sb = get_supabase()
    try:
        result = (
            sb.table("images")
            .select("id, client_id, label, storage_path, file_size_bytes, created_at")
    ...
    return {"generation_id": generation_id, "count": len(images), "images": images}
