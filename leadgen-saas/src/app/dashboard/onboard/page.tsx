...
type Phase = "loading" | "input" | "processing" | "complete";
  ...
  useEffect(() => {
    fetch("/api/brain")
      ...
      .then((data) => {
        if (data.brain) {
          setBrain({
            company_name: data.brain.company_name || "",
            services: data.brain.services || [],
            industries_served: data.brain.industries_served || [],
            target_customers: data.brain.target_customers || "",
            usp: data.brain.usp || "",
            proof_points: data.brain.proof_points || [],
            tone: data.brain.tone || "",
          });
          setWebsiteUrl(data.brain.website_url || "");
          setPhase("complete");
        } else {
          setPhase("input");
        }
            ...
            if (event.step === "complete" && event.brain) {
              ...
              fetch("/api/brain", {
                ...
                body: JSON.stringify({
                  ...
                  raw_website_content: event.raw_website_content || "",
                  raw_file_content: event.raw_file_content || "",
                }),
              }).catch(() => {});
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl shadow-card border border-border/30 p-6">
            ...
            <p className="text-xs text-text-tertiary mt-2">
              We&apos;ll scrape your website to understand your services, positioning, and tone.
            </p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-card border border-border/30 p-6">
            ...
            >
              ...
              <p className="text-xs text-text-tertiary">PDF brochures, catalogs, pitch decks, case studies</p>
              <input
                ref={fileInputRef}
                type="file"
                multiple
            ...
            {files.length > 0 && (
              <div className="mt-4 space-y-2">
                {files.map((file, i) => (
                  <div key={i} className="flex items-center justify-between px-4 py-2.5 rounded-xl bg-surface-secondary border border-border/50">
                    <div className="flex items-center gap-3">
                      <FileText className="w-4 h-4 text-primary" />
                      <span className="text-sm font-medium">{file.name}</span>
                      <span className="text-xs text-text-tertiary">{(file.size / 1024).toFixed(0)} KB</span>
                    </div>
                    <button onClick={() => removeFile(i)} className="p-1 hover:bg-surface-tertiary rounded-lg transition-colors">
      ...
      {phase === "processing" && (
        <div className="bg-white rounded-2xl shadow-card border border-border/30 p-8 animate-scale-in">
          <div className="text-center mb-8">
            ...
            <h2 className="text-xl font-bold mb-1">Building Your Company Brain...</h2>
            <p className="text-text-secondary text-sm">AI is analyzing your website and assets</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-card border border-border/30 p-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-text-secondary">
                  AI has analyzed your business. Here&apos;s what it understands.
                </p>
              </div>
            </div>
            ...
            <div className="grid gap-6">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-text-tertiary">Company</p>
                  <p className="text-lg font-bold">{brain.company_name || "—"}</p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-text-tertiary mb-2">Services</p>
                  <div className="flex flex-wrap gap-2">
                    ...
                    {brain.services.length === 0 && <span className="text-sm text-text-tertiary">No services detected</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-text-tertiary mb-2">Industries Served</p>
                  <div className="flex flex-wrap gap-2">
                    {brain.industries_served.map((ind, i) => (
                      <span key={i} className="px-3 py-1.5 rounded-full bg-accent/10 text-accent-dark text-xs font-medium">{ind}</span>
                    ))}
                    {brain.industries_served.length === 0 && <span className="text-sm text-text-tertiary">No industries detected</span>}
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-text-tertiary">Unique Selling Proposition</p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-text-tertiary">Target Customers</p>
                  <p className="text-sm mt-1">{brain.target_customers || "—"}</p>
                </div>
              </div>
              ...
              {brain.proof_points.length > 0 && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-text-tertiary mb-2">Proof Points</p>
              ...
              {brain.tone && (
                <div className="p-4 rounded-xl bg-surface-secondary border border-border/50">
                  <p className="text-xs font-semibold text-text-tertiary uppercase tracking-wider mb-1">Communication Tone</p>
